package com.github.toruishihara.simple_android_rtsp_viewer.pipeline
...
class HandLandmarkerHelper(
    ...
    fun setupHandLandmarker() {
        ...
        // Verify asset exists to prevent native crash
        try {
            context.assets.open(MP_HAND_LANDMARKER_TASK).close()
        } catch (e: Exception) {
            Log.e(TAG, "CRITICAL: Model file NOT FOUND in assets: $MP_HAND_LANDMARKER_TASK")
            handLandmarkerHelperListener?.onError("Model file missing from assets")
            return 
        }
        ...
        try {
            ...
            Log.e(TAG, "MediaPipe failed to load with a fatal error", e)
        }
    }
    ...
    fun detectImage(image: Bitmap): ResultBundle? {
        ...
        if (handLandmarker == null) {
            Log.e(TAG, "detectImage: handLandmarker is NULL. Model likely failed to load.")
            return null
        }
        ...
        try {
            ...
            Log.e(TAG, "MediaPipe detect exception", e)
        }

        // If handLandmarker?.detect() returns null or throws, indicate this.
        handLandmarkerHelperListener?.onError(
    ...
    companion object {
        ...
        private const val MP_HAND_LANDMARKER_TASK = "mediapipe/hand_landmarker.task"
